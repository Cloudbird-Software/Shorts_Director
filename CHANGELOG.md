...
### Added
- IR-0007 INV-5/BEH-5（E4 渲染侧）：renderer 真实媒体解码 + 信息层叠加——GENERATED/SHOT 轨 ffmpeg 抽帧归一化（fps 对齐 + letterbox，纯 Go 合成；占位纯色帧保留为测试模式 modes.placeholder_media）；信息层 drawtext（textfile 传参零转义；caption 经 block_ref 引用 Copy 文本）；R-3 安全区拒绝（锚点换算后越界即报错）；R-4 字体文件哈希核验 + 媒体短于时间线引用报错不静默续帧；AIGC 双轨（显式 aigc.disclosure overlay + 容器隐式元数据 use_metadata_tags，produceTime 取 plan.ScheduledDate 保持无时钟）；bitexact 双跑 digest 全等测试（真实解码+信息层+元数据）。
- IR-0007 IFACE-4/INV-6（E2 前置）：evals/ mock 商家数据集（3 商家：餐饮×2/美业×1，信息表全字段 + fictional 声明 + 授权占位记录）+ 合成种子图生成器（ffmpeg lavfi 确定性渐变，零依赖可重生成）+ 形态套件定义（form1_ambience 3 商家×K=5 / form4_digital_human 1 商家×K=5，断言包绑定形态 bindings；form1_smoke_fake 冒烟套件与 gen_i2v golden fixtures 咬合）；internal/eval/dataset_test.go 结构守护（套件受控校验、种子图存在性/PNG 魔数、垂类分布、冒烟套件 FakeRunner 真实执行）。
...
- Freeze Gate G2（双语言判定一致）：internal/entity/consistency_test.go 消费与 ajv 同批 G1 样本——valid 双侧必须全过；invalid 样本 Go Validate 放行者按 testdata/g2_go_pass_invalid.json 逐条登记（结构性豁免+理由，27 条），未登记放行/登记后拒绝均失败；TS 侧 tests/consistency.test.ts 防登记表腐烂（条目须对应真实样本、理由非空）。
...
- Freeze Gate G7：提交首批 golden fixture（testdata/golden/{probe,blackdetect_ratio}，GoldenKey 内容寻址）；golden 字段清单静态检查——AST 扫描 internal/ 源码 `Outputs["字面量"]` 访问 ⊆ fixture 清单，消费包以 `ConsumedGoldenOps` 声明锚点（qc 桥接器），fixture 删字段/越界访问两条负例必须失败；fixture 经 FakeRunner 与 qc 真实消费路径命中（防腐烂）。
