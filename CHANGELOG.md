...
- Freeze Gate G9/G10（二）：planner 时长求解属性 ×1000（可行输入下帧数守恒 IV-VP-1、时间线无缝连续、speed 工艺上限、同输入确定性）；slotquery 属性 ×1000（随机 query×池 must/forbid 语义不变量、终端兜底可解 IV-SQ-1、Resolve 确定性）与变形不变式 M2（池追加候选→既有命中相对排名稳定）。
